...
> **Files:** `renderer.h`, `renderer.cpp`, `caption_parser.{h,cpp}`, `caption_queue.cpp`, `caption_texture.cpp` — plus `banner_config.h`
...
- `CaptionEntry::isSfx` marks SFX for each queued entry (not for the entire `<sb>`-split caption).
...
- SFX queue handling: `SetCaptionText` flags `isSfx` for each `<sb>` part containing exact `<sfx>`; `UpdateQueue` makes SFX fully opaque immediately after its scheduled start (no fade-in), while keeping the normal fade-out.
...
```
hkProcess → if cc_subtitles != 0 and raw contains exact "<sfx>": drop whole caption
          → otherwise SetCaptionText (m_CS)
  split raw on "<sb>" → flag isSfx per part → ParseCaptionText per part (shared CaptionFormatState; strips <sfx>)
  distribute duration ∝ GetVisibleCharCount (UTF-8 code-point count) → push CaptionEntry

hkPresent (render thread, m_CS):
  UpdateQueue(dt)
    expire old entries
    after start: SFX opacity jumps to 1; other entries fade in; both fade out normally
...
- `hkProcess` filters the **whole raw caption** before the queue sees `<sb>` parts: when SFX is hidden, even a mixed caption with one `<sfx>` part is dropped in full. If the ConVar cannot be found, the raw caption reaches the queue as usual. F11 hides all captions separately.
