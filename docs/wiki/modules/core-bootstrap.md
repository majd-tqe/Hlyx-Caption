...
> Files: `proxy.cpp`, `hooks.cpp` (575 lines), `wininet.def`, `version.rc`/`resource.h`
...
- Bootstrap the mod on a background thread: install crash diagnostics, acquire DPI awareness, resolve `g_ModDirA`/`g_IniPath`, load `resources/settings.ini`, initialize MinHook, hook user32 cursor APIs early, sig-scan `client.dll` for the caption delivery function, and finally hook DXGI `Present`/`Resize`.
...
- The caption signature (`F3 0F 11 5C 24 ? 48 89 54 24` in `client.dll`, hooks.cpp:473) is fragile. There is exactly one pattern in the code, and no `+0x58` offset anywhere. If captions vanish after a game update, that single pattern is the first thing to re-check. Earlier docs mentioned a second pattern and a ConVar offset; neither exists in the tree, so they were removed.
- `version.rc` holds the Windows DLL version (`0.9.4.0`); keep the package versions in `package.json` and `vcpkg.json` (`0.9.4`) synchronized when releasing.
