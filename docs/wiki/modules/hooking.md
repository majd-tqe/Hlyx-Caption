...
- Call sites:
  - `hooks.cpp:MainThread` — `MH_Initialize`, then the three user32 hooks via `MH_CreateHookApi(L"user32", ...)`, then the caption hook via `MH_CreateHook(foundAddr, hkProcess, &oProcess)` where the target comes from a `client.dll` signature scan with a 60x500 ms retry loop. It also discovers the `cc_subtitles` global pointer near the caption function in `client.dll`'s `Process`. The hook bodies (`hkSetCursorPos`/`hkClipCursor`/`hkSetCapture`) live in `renderer.cpp`; only the installation lives in `hooks.cpp`. One detail to get right: `MH_EnableHook(MH_ALL_HOOKS)` appears exactly once, right after the `SetCursorPos` hook succeeds. There is no enable call after the `ClipCursor`/`SetCapture` creations. So describing all three as enabled immediately overstates it.
  ...
  - Caption hook behavior: `hkProcess` reads `stream` and checks the raw text before forwarding it to the renderer. When the live `cc_subtitles` value is nonzero and the raw text contains exact `<sfx>`, it drops the whole caption (including any non-SFX `<sb>` parts). If the ConVar pointer is unavailable, SFX remains visible. It never calls `oProcess`; the original function is dropped entirely, not chained through a trampoline.
...
- Bootstrap also finds the `cc_subtitles` ConVar pointer by signature in `client.dll`'s `Process`; `IsSfxHidden()` reads its integer at `+0x58` on each check, rather than caching the setting or reading the mod INI. A failed scan or null pointer leaves SFX visible.
